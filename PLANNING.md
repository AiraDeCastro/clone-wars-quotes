...
- **Platform-native widgets over a cross-platform widget framework.** WidgetKit, Jetpack Glance, and the Windows Widgets Board API are different enough (layout models, refresh budgets, render targets) that a shared cross-platform widget UI layer would fight each OS more than it would save work. Within Apple's platforms specifically, WidgetKit/SwiftUI code is genuinely shared as-is — `ios/ColdOpen` and `ios/ColdOpenWidget` compile the identical source into both the iOS and macOS targets, no `#if os(...)` branches. Across the iOS/Android/Windows boundary, only the corpus and selection *logic* are shared (same schema, same algorithm) — the widget *rendering* is platform-specific there.
