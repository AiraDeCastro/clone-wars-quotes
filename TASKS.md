...
- [ ] **(newly discovered)** The Windows logo sizes (`StoreLogo` 50x50, `Square150x150Logo`, `Square44x44Logo`) follow the classic UWP convention from memory, not a fetched/verified current source — re-check against current Store packaging requirements before real submission (same discipline already applied to the WindowsAppSDK version pin, just not done yet for these).
