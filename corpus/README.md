...
**One conflict with the original seed entry, now resolved:** the pre-existing seed entry attributed *"Heroes are made by the path they choose, not the powers they are graced with."* to "Rookies" (S1E5) with `arc: "Malevolence"`. The new source gives Rookies' actual line as *"The best confidence builder is experience,"* with `arc: "Rookies"` (standalone — the Malevolence arc is Season 1 episodes 2-4 only). The seed entry was written before any real transcript source existed, specifically as a schema example. **Confirmed by the user as correct** — the new source's version is the one in `quotes.json`.
