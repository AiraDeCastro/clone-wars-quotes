...
- **Accessibility: quote + attribution now read as one VoiceOver label.** The quote `Text` carries an explicit `.accessibilityLabel(...)` combining both (spoken-form attribution, not the visual "S1E5" abbreviation); the attribution `Text` is `.accessibilityHidden(true)` so it isn't announced twice. Standard SwiftUI APIs, but still not confirmed on a real device/VoiceOver.
- **Favorites are wired but unverified.** `FavoritesStore` reads/writes the same App Group suite `QuoteSelector`'s persistence uses — so it inherits the exact same "not registered yet" gap described above (favoriting still works locally in the app, it just won't affect the widget's randomize pool until Signing & Capabilities is set up). The App Group identifier itself moved out of `ColdOpenWidget.swift`'s private enum into a new public `AppGroup` type in `ColdOpenCore`, since both the widget extension and the app target now need it and they're separate compiled modules.
- **Companion browser replaces the old single-quote placeholder.** `ContentView.swift` now shows the full corpus in a searchable `List` (filtering by episode title, arc, or quote text), using `ContentUnavailableView`/`.search(text:)` for empty/error states. Standard, well-established SwiftUI (`List`, `.searchable`, `NavigationStack`) rather than anything WidgetKit/Glance-specific, so lower risk than most of this session's platform code — but still not opened in Xcode to confirm it actually renders.
- **Scheduled auto-refresh is wired but unverified.** `ColdOpenWidgetProvider.getTimeline` now pre-computes 6 entries 4 hours apart with reload policy `.atEnd`, instead of a single `.never` entry. No Swift toolchain to confirm the timeline actually fires as scheduled — that's also genuinely subject to WidgetKit's own background-refresh budget on top of anything this code does, so "works" here really means "gives the OS a reasonable schedule to work with," not a guaranteed cadence.
- **Large widget size added, no faction emblem yet.** `.systemLarge` is now in `supportedFamilies`, and `ColdOpenWidgetView` scales its type/spacing up via `@Environment(\.widgetFamily)`. The faction emblem CLAUDE.md's design direction calls for at this size is deliberately not implemented — it needs a design decision and a quote-to-faction mapping the corpus schema doesn't have, both logged in TASKS.md rather than guessed at.
- **Instagram Stories direct-share is implemented but inert.** `InstagramStorySharing.swift` writes the rendered card to `UIPasteboard` under Instagram's documented key and opens `instagram-stories://share` — but Instagram requires a registered Facebook App ID (as `source_application`) and rejects the share without one, per Meta's own docs. `facebookAppID` is a deliberate blank placeholder; until it's filled in with a real ID, `isAvailable` stays `false` and the app falls back to the generic share sheet above. Fill it in once an app is registered at developers.facebook.com.
