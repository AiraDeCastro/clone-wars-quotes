...
- **All three widget sizes (small/medium/large) are now declared**, matching the parity already shipped on iOS/Android. `Package.appxmanifest` gained `<Capability><Size Name="large" /></Capability>`, and `QuoteCardTemplate`'s Adaptive Card JSON now has three parallel `$when`-gated quote `TextBlock`s keyed on `$host.widgetSize` (large gets `extraLarge` text) instead of one unconditional block — the documented Widgets Board pattern for per-size content, fetched and confirmed from Microsoft's C# widget provider doc rather than guessed at. No C# provider code changes needed; size handling lives entirely in the JSON template. The embedded JSON was extracted and validated with Python's `json.loads`, not just brace-counted. Same faction-emblem deferral as iOS/Android (design decision + missing quote-to-faction mapping) applies here too.
- **No-repeat selection now persists across a provider restart**, via `WidgetUpdateRequestOptions.CustomState` — see `QuoteSelector`'s and `ColdOpenWidgetProvider`'s doc comments. `CustomState` stores full quote text (not yet an index into `corpus/quotes.sqlite`), which will grow non-trivially once the real ~130-quote corpus exists — logged in TASKS.md as a follow-on, not solved here.
- **App icons done, screenshot still missing.** `StoreLogo.png`, `Square44x44Logo.scale-{100,200,400}.png`, `Square150x150Logo.scale-{100,200,400}.png`, and `ProviderAssets/ColdOpen_Icon.png` all exist now (generated via `scripts/gen-app-icon.py` at the repo root — the same design as iOS/Android). Base sizes and the scale-qualifier naming convention were checked against Microsoft's current ["Construct your Windows App's Icon"](https://learn.microsoft.com/windows/apps/design/iconography/app-icon-construction) doc, not assumed — `Square44x44Logo`/`Square150x150Logo` now have the documented minimum 100/200/400% scale variants; `StoreLogo` stays a single 100%-scale file. `ProviderAssets/ColdOpen_Screenshot.png` is deliberately still missing — see `ProviderAssets/README.md`.
- **Not the full Store-required asset set.** The same doc lists a much larger required set for actual Microsoft Store publication — AppList target-size icons (14 sizes × 3 theme variants), tile assets, a splash screen, `StoreLogo` scale variants. None of that is built, and shouldn't be until the distribution-plan decision in TASKS.md's open items is actually made — building it now would be speculative work for a channel that might not be used.
